        ...
        if (d.result == 0) {
            ...
            new Vue({
                ...
                created: function () {
                    ...
                    var list_arr = "", // 请求书籍列表
                        toc, // 存放加入加入书架的书籍的章节列表
                        lastChapter, // 存放已读的章节数
                        fiction_array_info = []; // 存放每个书籍的已读章节信息
                    if (fiction_array.length > 0) {
                        ...
                        for (var i = 0; i < fiction_array.length; i++) {
                            var obj = {}, fiction = fiction_array[i];
                            list_arr += fiction + ',';
                            // 获取每个书籍的已读章节信息
                            toc = JSON.parse(getValue('_toc', fiction));
                            lastChapter = parseInt(getValue('_last_chapter', fiction));
                            var obj = {};
                            toc && lastChapter ? obj[fiction] = toc[lastChapter]['title'] : obj[fiction] = '';
                            fiction_array_info.push(obj);
                        }
                        console.log(fiction_array);
                        $.ajax({
                            url: '/ajax/fictions_list',
                            type: 'get',
                            data: {
                                ids: list_arr.replace(/\,$/, ""),
                                owner: '0,2'
                            },
                            dataType: 'json',
                            success: function (res) {
                                if (res.result == 0) {
                                    if (res.items.length == fiction_array_info.length) {
                                        for (var i = 0; i < res.items.length; i++) {
                                            res.items[i]['last_chapter_title'] = fiction_array_info[i][fiction_array[i]];
                                        }
                                    }
...
function getValue(key, value) {
    return localStorage.getItem('ficiton_reader_' + value + key);
}